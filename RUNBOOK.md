...
회계 채널에서 `/invite @snusv-bot`. 스코프만으로는 히스토리를 못 읽는다.

### 3. 회계 User Group 만들기

**User Group은 Slack 유료 플랜 기능이다.** 생성 권한은 기본적으로 워크스페이스 소유자·관리자에게 있다.

1. 좌측 사이드바 **홈** → **디렉터리(Directories)** → **사용자 그룹(User Groups)**
2. **사용자 그룹 만들기**
3. **이름**과 **핸들** 입력
   - 이름: `39기 회계` 처럼 사람이 읽을 이름
   - **핸들**: `accounting` 등. 이 값을 `@` 없이 `ACCOUNTANT_USERGROUP` 에 넣는다
4. 목적·기본 채널은 비워도 된다 → **다음**
5. **멤버 추가**에서 이번 기수 회계 담당자 선택 → **그룹 만들기**

멤버 교체는 같은 경로에서 그룹 선택 → **멤버 편집** → 저장.

**기수가 바뀌면 이 그룹의 멤버만 교체한다.** 그래서 코드를 안 건드려도 된다.

> 디렉터리에 "사용자 그룹" 메뉴가 없으면 무료 플랜이다. 그 경우 User Group 대신 담당자 ID를 직접 나열하는 방식이 필요하며, 코드 수정이 따른다.
